...
race_first <- post_1980_multiple_apps %>%
  ...
  select(year_cycle_first = year_cycle, race_first = race, ssn) %>% 
  ungroup()
...
race_last <- post_1980_multiple_apps %>%
  ...
  select(year_cycle_last = year_cycle, race_last = race, ssn) %>% 
  ungroup()
...
```{r}
## CenSoc-Numident ABE
matched_census_bunmd <- fread("/home/ipums/ugury-ipums/censoc-abe-implementation/data/matched-census-bunmd/matched_census_bunmd.csv") %>% 
...
## write out files 
fwrite(analysis_dataset_with_covariates, "../data/analysis_dataset_with_covariates.csv")

fwrite(analysis_dataset, "../data/analysis_dataset.csv")
```
